...
var addRandom = function (data) {
  for (var index in data) {
    if (data.hasOwnProperty(index) && !isNaN(data[index])) {
      var r = Math.random()
      if (r > 0.5) { data[index] += r * 2 } else { data[index] -= r * 2 }
    }
  }
  return data
}
...
var initSocket = function (domain, resource, apiKey, apiSecret) {
  socket = new WebSocket(`wss://data-staging.waylay.io/resources/` + resource + '/socket?domain=' + domain + '&apiKey=' + apiKey + '&apiSecret=' + apiSecret)
  ...
  socket.onmessage = function (msg) {
    ...
    metricsData.forEach(function (metric) {
      if (!metricsSocketData[i]) { metricsSocketData[i] = [] }
  ...
  if (domain && domain !== '') {
    ...
    if (password && resource && key) {
      WAYLAY.getCurrentObject(domain, key, password, resource, function (data) {
        ...
        if (index !== -1) { metricsData.splice(index, 1) }
...
$(document).ready(function () {
  ...
  var successHandler = function (info) {
    var h3 = '<h3>Output</h3>'
    var textarea = `<textarea id="info">${info}</textarea>`
    $('#output').empty()
    $('#output').append(h3).append(textarea)
    $.toast({
      heading: 'Success',
  ...
  $('#pushDataTemplate').click(function (e) {
    ...
    var parallel = $('#parellel').is(':checked')
    var executeActuators = $('#executeActuators').is(':checked')
    try {
      var data = JSON.parse($('#data').val())
      if (data.resource === undefined) { data.resource = resource }
      var dataToSend = {
        data: [[data]],
        conf: {
          parallel: parallel,
          resource: resource,
          executeActuators: executeActuators
        }
      }
      var domain = $('#domain').val()
      var key = $('#key').val()
  ...
  $('#save').click(function (e) {
    e.preventDefault()
    var domain = 'staging.waylay.io'
    var key = $('#key').val()
    var secret = $('#secret').val()
    var resource = $('#resource').val()
    var availableTags = []
    WAYLAY.getAllTemplates(domain, key, secret, function (data) {
      data.map(item => {
  ...
  $('#startSimulation').click(function (e) {
    ...
    if (simulationLineChart && simulationLineChart.datasets && simulationLineChart.datasets.length > 0) {
      while (simulationLineChart.datasets[0].points.length > 0) { simulationLineChart.removeData() }
    }
  ...
  $('#sendBatch').click(function (e) {
    ...
    var executeActuators = $('#executeActuators').is(':checked')
    if (key && password && domain) {
      if (simulationBatchData.length > 0 && template) {
        var data = []
        simulationBatchData.forEach(function (d) {
          if (d.resource === undefined) { d.resource = resource }
          data.push([d])
        })
        var dataToSend = {
          data: data,
        ...
        WAYLAY.pushDataToTemplate(domain, key, password, dataToSend, template, function (info) {
          ...
          // $('#info').text('DONE: Processed rows:' + data.length + ', Execution time: ' + time + '[sec]')
          var alert = `<div class="alert alert-success"> Processed rows: ${data.length}, Execution time: ${time}[sec]</div>`
          $('#batchoutput').empty()
          $('#batchoutput').append(alert)
          $.toast({
            heading: 'Success',
            text: 'Batch Request successfull',
            showHideTransition: 'slide',
            icon: 'success'
          })
        })
  ...
  var readCsv = function (file) {
    ...
    reader.onload = function (e) {
      ...
      try {
        ...
        simulationBatchData = buildSimulationData(csv.dataRows, csv.columnNames)
